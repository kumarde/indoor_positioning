...
%
%   Loss layer::
%     The loss layer (log loss, softmax log loss, multiclass hinge loss, 
%     multiclass structured hinge loss, binary log loss, Logistic log loss
%     and hinge loss) wraps VL_NNLOSS(). It has fields:
%
%     - layer.type = 'loss'
%     - layer.class: the ground-truth class.
%     - layer.opts: the parameters for specific loss
%
...
for i=1:n
    ...
    switch l.type
        ...
        case 'loss'
            res(i+1).x = vl_nnloss(res(i).x, l.class, [], l.opts) ;
        case 'softmaxloss'
        ...
        case 'pdist'
            nClass = size(res(i).x, 3);
            batchSize = size(res(i).x, 4);
            labels = full(sparse(1:batchSize, double(l.class), 1, batchSize, nClass))';
            labels = single(reshape(labels, 1, 1, nClass, batchSize));
            res(i+1).x = vl_nnpdist(res(i).x, labels, l.p, [], 'noRoot', l.noRoot, 'epsilon', l.epsilon) ;
        case 'custom'
...
if doder
    ...
    for i=n:-1:max(1, n-opts.backPropDepth+1)
        ...
        switch l.type
            ...
            case 'loss'
                res(i).dzdx = vl_nnloss(res(i).x, l.class, res(i+1).dzdx, l.opts) ;
            case 'softmaxloss'
            ...
            case 'pdist'
                ...
                labels = full(sparse(1:batchSize, double(l.class), 1, batchSize, nClass))';
                labels = single(reshape(labels, 1, 1, nClass, batchSize));
                res(i).dzdx = vl_nnpdist(res(i).x, labels, l.p, res(i+1).dzdx, ...
