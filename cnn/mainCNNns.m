...
%
% Put the decompressed files to "toolbox/matconvnet-1.0-beta16/"
%
...
addpath(genpath('../toolbox/vlfeat-0.9.20/toolbox/mex'));
addpath('../toolbox/matconvnet-1.0-beta16/examples');
addpath('../toolbox/vlfeat-0.9.20/toolbox/misc');
addpath('../toolbox/matconvnet-1.0-beta16/matlab');
rmpath('../toolbox/vlfeat-0.9.20/toolbox/misc');
...
for i = 1 : nTrain
    for color = 1 : 3
        avg = mean(trainData(i, color : 3 : end));
        tempData = trainData(i, color : 3 : end) - avg;
        normResult = norm(tempData);
        train_data_normalized_standardized(i, color : 3 : end) = tempData/normResult*10;
    end
end
for i = 1 : nVal
    for color = 1:3
        avg = mean(valData(i, color : 3 : end));
        tempData = valData(i, color : 3 : end) - avg;
        normResult = norm(tempData);
        val_data_normalized_standardized(i, color : 3 : end) = tempData/normResult*10;
    end
end
